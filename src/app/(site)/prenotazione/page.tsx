...
import {
  Calendar, Users, User, Mail, Phone, MessageSquare,
  CreditCard, ShieldCheck, ArrowRight, Info, CheckCircle2
} from 'lucide-react';
...
function BookingForm() {
  ...
  useEffect(() => {
    ...
    if (differenceInDays(parseISO(form.check_out), parseISO(form.check_in)) < 1) return;

    const fetchPrice = async () => {
      setLoadingPrice(true);
      try {
        const res = await fetch(`/api/prices?check_in=${form.check_in}&check_out=${form.check_out}`);
        if (!res.ok) throw new Error('Errore nel calcolo prezzi');
    ...
    try {
      const bookingRes = await fetch('/api/bookings', {
        ...
        body: JSON.stringify({
          ...
          deposit_amount: priceData.deposit,
          deposit_paid: 0,
        }),
      });

      const { booking, error } = await bookingRes.json();
      if (!bookingRes.ok || error) throw new Error(error || 'Errore creazione prenotazione');
  ...
  const amountToPay = paymentType === 'deposit' ? priceData?.deposit : priceData?.total;
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      ...
      <div className="flex items-center justify-center gap-2 mb-10">
        {[
          ...
          { n: 2, label: 'I tuoi dati' },
          { n: 3, label: 'Pagamento' },
        ].map(({ n, label }, i) => (
          <div key={n} className="flex items-center gap-2">
            <div className={`flex items-center gap-2 ${step >= n ? 'text-gold-600' : 'text-gray-300'}`}>
              <div className={`w-7 h-7 flex items-center justify-center text-sm font-bold transition-colors duration-300 ${
                step > n ? 'bg-green-500 text-white' : step === n ? 'bg-gold-500 text-white' : 'bg-gray-200 text-gray-400'
              }`}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      min={form.check_in || format(new Date(), 'yyyy-MM-dd')}
                      required
                      className="input-field"
                    />
                  </div>
              ...
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    value={form.message}
                    onChange={handleChange}
                    placeholder="Eventuali richieste, animali domestici, orario di arrivo..."
                    rows={3}
              ...
              <div className="space-y-4 mb-6">
                <label className={`flex items-start gap-4 p-5 border-2 cursor-pointer transition-all ${
                  ...
                  <div>
                    ...
                    <div className="text-2xl font-display text-gold-600 my-1">
                      {priceData ? formatCurrency(priceData.deposit) : '—'}
                    </div>
                    <div className="text-xs text-gray-500">
                      Saldo di {priceData ? formatCurrency(priceData.remaining) : '—'} da pagare all'arrivo
                    </div>
                  </div>
                </label>
                ...
                <label className={`flex items-start gap-4 p-5 border-2 cursor-pointer transition-all ${
                  ...
                  <div>
                    ...
                    <div className="text-2xl font-display text-gold-600 my-1">
                      {priceData ? formatCurrency(priceData.total) : '—'}
                    </div>
                    <div className="text-xs text-green-600 font-bold">
                      ✓ Nessun pagamento richiesto all'arrivo
                    </div>
                  </div>
                </label>
              </div>
                ...
                >
                  {loading ? (
                    ...
                    <>
                      ...
                      Paga {amountToPay ? formatCurrency(amountToPay) : ''}
                    </>
                  )}
                </button>
              </div>
          ...
          <div className="bg-cream-100 border border-gold-200 p-6 sticky top-24">
            ...
            {form.check_in && form.check_out && (
              <div className="space-y-2 text-sm mb-4">
                <div className="flex justify-between">
                  ...
                  <span className="font-bold">{format(parseISO(form.check_in), 'dd MMM yyyy', { locale: it })}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Check-out</span>
                  <span className="font-bold">{format(parseISO(form.check_out), 'dd MMM yyyy', { locale: it })}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Ospiti</span>
                  <span className="font-bold">{form.guests_count}</span>
                </div>
              </div>
            )}
            ...
            {loadingPrice ? (
              <div className="space-y-2 animate-pulse">
                {[1, 2, 3].map((i) => <div key={i} className="h-5 bg-cream-200 rounded" />)}
              </div>
            ) : priceData ? (
